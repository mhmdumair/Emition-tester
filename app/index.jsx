...
export default function App() {
  ...
  const signIn = async ()=>{
    try {
      ...
      const users = await getAllUser();
      const user = users.find(data => 
        data.vehicleNumber === formValue.vehicleNumber && data.nic === formValue.nic && data.name === formValue.name);
      ...
      <View className="w-full justify-center items-center min-h-[85vh] px-4 my-6">
        ...
        <View className="my-2">
          <Text className= "text-white text- text-[18px] mt-3">
            if you are new here?{"  "} <Link href='/sign-up' className="text-secondary-100 ">Sign Up</Link>
          </Text>
        </View>
